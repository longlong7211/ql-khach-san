...
export default async function handler(
  ...
  if (username === "admin" && password === "123abc") {
    return res.status(200).json({ token: "tokenadmin", message: "Đăng nhập thành công", role: 'admin' })
  } else
    if (username === "user" && password === "123abc") {
      return res.status(200).json({ token: "tokenuser", message: "Đăng nhập thành công", role: 'user' })
    } else {
